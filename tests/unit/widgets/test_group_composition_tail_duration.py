...
def test_the_header_box_writes_the_last_frames_duration(widget, fired):
    widget._tail_spin[widget.root].setValue(2000)

    assert widget.gm.get_group(widget.root).entries[1].duration_ms == 2000
    assert widget.gm.get_group(widget.root).entries[0].duration_ms is None
    assert fired, "the edit has to be undoable and repaint the preview"
...
def test_editing_the_header_box_moves_the_last_frames_row(widget):
    widget._tail_spin[widget.root].setValue(2000)
