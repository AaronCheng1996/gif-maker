...
把數個片段接成一個檔案播放。這不是剪輯器——沒有時間軸、不能修剪、沒有轉場。加入 GIF 和影片、
排好順序、拼接。

真正的工作在於各段規格不一致時要怎麼辦，因為 ffmpeg 的 concat filter 會拒絕尺寸、像素格式或
取樣長寬比不同的輸入，而由 GIF、手機影片和螢幕錄影組成的序列，這三項通常沒有一項相同：

- **尺寸**採補邊，絕不拉伸。形狀不同的片段會被等比縮放放進輸出畫面，其餘部分補上背景色——
  為了把數字對齊而把 4:3 的片段拉成 16:9，會讓畫面裡的所有人都變形。畫面尺寸預設比照第一個
  片段，也可以選擇容納最大的片段。
- **取樣長寬比**在每一段都強制為 1:1，因為只要有一個檔案宣告非方形像素，即使儲存的尺寸正確，
  整段拼接播放起來仍會被壓扁。
- **影格率**預設往高的取，而不是往低的取，讓最流暢的片段保有它的動態，而不是被降到最差的那一段。
- **聲音**只有在每一段都有的情況下才會保留。把無聲片段接到有聲片段上，從接縫之後就會失去同步，
  所以是全有或全無，而摘要會說明實際是哪一種。

輸出為 MP4（H.264）或 GIF，兩者都會疊在一個純色底上。GIF 原本想用透明補邊，但 GIF 對透明像素的
處理方式是「顯示它底下原本的東西」，結果直式片段兩側的黑邊填的是前一段的背景，而不是空白。

點選片段會以拼接後實際的畫面來預覽它，連補邊一起顯示——有沒有補邊，用眼睛看遠比從設定面板推敲
容易得多。
...
src/
  ...
  core/
    ...
    concat.py                   將片段首尾相接；先統一尺寸、長寬比與影格率
  ...
  widgets/
    ...
    video_concat_widget.py      影片拼接工具介面（可排序的片段清單、補邊預覽）
