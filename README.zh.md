...
- 可直接在群組標題列調整最後一幀的時間——那一幀就是重新循環前的停頓，
  不必再捲到最底下才能改
- 透過 SubGroupEntry 將群組嵌套進其他群組，並設定獨立的循環次數與偏移量
- 透過 LayerBlockEntry 進行多圖層合成（每幀合成多個時間軸）
- 群組可展開或收合；被引用的群組只顯示為一列標題，並附有跳到該群組本體區塊的按鈕，
  因此展開某個群組不會連帶展開它在別處的引用
...
把數個片段接成一個檔案播放。仍然不是剪輯器——沒有軌道、沒有轉場、沒有特效——但有兩件拼接時
一直會用到的事，值得佔這個版面。

**素材庫和時間軸是分開的。** 加入檔案和使用檔案是兩回事，所以一邊是素材清單、另一邊是使用清單。
同一個片段因此可以在時間軸上出現多次——片頭拿來當片尾、同一顆鏡頭剪成前後兩段——這是單一份路徑
清單根本表達不了的。從素材庫雙擊或拖曳即可放到時間軸；把素材移出素材庫，不會動到已經放上去的片段。

**每個片段有自己的起訖點。** 大多數片段都要先去頭去尾才接得乾淨，而先跑去別的工具處理，會把
一個步驟變成三個。裁切是針對「這一次使用」而不是針對檔案，所以同一份素材在不同位置可以有不同的
剪法。有裁切的片段會先 seek 再解碼，所以從一小時的錄影裡取四秒，就只讀那四秒。

裁切介面是一條**膠片**，不是兩個數字。真正要問的事情——這個片段是不是要跳過開頭的場記板、是不是
要在鏡頭甩開之前結束——本來就是關於畫面的問題，所以控制項就是畫面本身：整段片子橫向攤開，保留的
範圍維持原亮度、被丟掉的頭尾變暗，兩端各有一個把手，還有一條播放頭。數字輸入框仍然保留，因為畫面
不擅長表達精確的數值。

**預覽會把整段拼接跑一次**，在寫出任何檔案之前，而且是每一格都播。直覺的做法——直接翻膠片條那批
影格——實際上稀疏到沒辦法看，因為那批影格是攤在整段素材上取樣的：一分鐘的片子只剩每秒兩格。但把
真實影格率的畫面留在記憶體裡同樣不可行，10 秒的 480×360 就要大約 160 MB。

所以預覽就是拼接本身，只是算得又小又快——寬 640px、ultrafast、crf 30——然後當成檔案播放。12 秒的
拼接算圖不到半秒，出來是真實的 25fps，而不是膠片條的 12fps。它走的是和輸出完全相同的管線，所以
順序、裁切、取景和補邊都是真的；只有「完整尺寸下的編碼品質」還是得看實際檔案。時間軸沒有變動時會
直接重播已經算好的檔案，不會重算。若環境缺少 QtMultimedia，則退回逐格翻看取樣影格的做法。

**對齊前一個片段**會在這個片段裡搜尋和前一段結尾接得最順的那一格，並從那裡開始。相似度是三個量測
而不是一個，因為接縫會壞的方式有三種：顏色抓得到換場景，邊緣能在緩慢的曝光漂移下存活（那種漂移會
把純像素比對整個淹沒），而拿每一格和它自己的下一格比較，則抓得到「兩格明明一模一樣，但一格在鏡頭
運動中、另一格是靜止」的情況。結果用文字回報——完全連貫、接近、找不到合適的畫面——因為那個分數只有
對照它被校正的門檻才有意義。

其餘的部分都是在處理各段規格不一致的情況，因為 ffmpeg 的 concat filter 會拒絕尺寸、像素格式或
取樣長寬比不同的輸入，而由 GIF、手機影片和螢幕錄影組成的序列，這三項通常沒有一項相同：

- **尺寸**採補邊，絕不拉伸。形狀不同的片段會等比縮放放進輸出畫面，其餘補上背景色——為了把數字
  對齊而把 4:3 拉成 16:9，會讓畫面裡的所有人都變形。畫面尺寸預設比照第一個片段，也可以選擇容納
  最大的片段。
- **取樣長寬比**在每一段都強制為 1:1，因為只要有一個檔案宣告非方形像素，即使儲存的尺寸正確，整段
  拼接播放起來仍會被壓扁。
- **時間戳**每段都歸零。concat 預期每個輸入都從零開始，而從檔案中間切出來的片段，本來會帶著它在
  原檔裡的時間戳。
- **影格率**預設往高的取，讓最流暢的片段保有它的動態，而不是被降到最差的那一段。
- **聲音**只有在每一段都有的情況下才會保留，並且會重新取樣成統一格式，因為 concat 對聲音的挑剔
  程度和對影像一樣。把無聲片段接到有聲片段上，從接縫之後就會失去同步，所以是全有或全無，而摘要
  會說明實際是哪一種。

輸出為 MP4（H.264）或 GIF，兩者都會疊在一個純色底上。GIF 原本想用透明補邊，但 GIF 對透明像素的
處理方式是「顯示它底下原本的東西」，結果直式片段兩側的黑邊填的是前一段的背景，而不是空白。

預覽會以拼接後實際的畫面顯示目前播放頭所在的影格，連補邊一起，並標示播放頭是在保留範圍內還是
已被裁掉。
...
- 批次處理時裁切框維持**像素尺寸**而非比例，所以不同尺寸的來源仍會輸出成一致大小；同時每個檔案各自記住自己的框位置，調整其中一個不影響其他（取消勾選「Same size for every file」則連尺寸也各自獨立）
- 預設在來源旁輸出 `<檔名>_cropped.gif`；也可指定輸出資料夾，或在確認後直接覆蓋原檔

像素欄位在離開欄位時才套用，而不是每敲一鍵就套用：逐鍵更新時，把 200 改成 1000 會先變成
1000200、被夾到畫面大小，最後兩個數字都不是。
...
src/
  ...
  core/
    ...
    concat.py                   將時間軸片段首尾相接；先裁切，再統一尺寸、長寬比與影格率
    seam.py                     尋找和某一格接得最順的畫面（顏色、邊緣、動態）
  ...
  widgets/
    ...
    video_concat_widget.py      拼接工具介面（素材庫、時間軸、膠片式裁切、預覽播放）
    trim_bar.py                 膠片條，含可拖曳的起訖把手與播放頭
